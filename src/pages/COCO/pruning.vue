...
<!--      the choice of whether to employ sparse learning will determine the available pruner-->
    ...
    <div v-show="finetune==='True'">
      <div style="margin: 0 0 20px 0; text-align: left;">
        <label style="margin-left: 50px; font-size:18px; border: 0; color: black; word-wrap: break-word; white-space: pre-wrap;">Step7: Choose experimental environment (CIFAR experiments do not need DDP acceleration).</label>
      </div>
      <div style="margin: 0 0 20px 0; text-align: left;">
      ...
      <div v-show="finetune==='True'">
        ...
        <div style="margin: 0 0 20px 0; text-align: left;">
          <label style="margin-left: 50px; font-size:18px; border: 0; color: black; word-wrap: break-word; white-space: pre-wrap;">Step8: Input the number of epochs for finetuning (Integer required and 100 is recommend, other hyperparameters have been set to optimal).</label>
        </div>
        <div style="margin: 0 0 20px 0; text-align: left;">
          <el-input
              v-model="epoch"
...
<!--  叶子节点对话框-->
  <el-dialog class="scrollable" v-model="LeafNodeVisible" style="width:20%; height:30%; text-align: center; overflow: auto; font-family: Arial;"
             :draggable="true" @close="LeafNodeVisible = false" :append-to-body="true" title="Confirm your task" >
<!--    <div style="display: flex; text-align: center; flex-direction: column; width: 30%; justify-content: space-around">-->
<!--      <el-button type="warning">Upload a raw model</el-button>-->
<!--      <el-button type="success">Prune the pretrained model</el-button>-->
<!--      <el-button type="primary">Prune a user's model</el-button>-->

<!--    <p>Child options expand when hovered</p>-->
    <p v-if="!store.state.isAut" style="color: darkred"> You need to log in first before <br> submitting your task. </p>
    <el-cascader
...
const finetunemode = ref('DDP')
const INtype = ref('cnn')
const PrunedialogVisible = ref(false);
const LeafNodeVisible = ref(false);
...
const handleCascaderChange = async (value) => {
  ...
  if(value[0].type=='upload'){
    ...
    uploadType.value = ''
    modelName.value = cascaderModel
    usrModelName.value = ''
    console.log("d.parent.parent.name", cascaderDataset)
    if(cascaderDataset==="CIFAR10"||cascaderDataset==="CIFAR100"||cascaderDataset==="ImageNet"){
      structureBeforePruned.value = '/nfs/lhl/Torch-Pruning/benchmarks/model_before_pruned/test/'+
          cascaderDataset.toLowerCase()+'_'+cascaderModel.toLowerCase().replace('(tiny)','').replace('-','_')+'_'+'Pretrained.log'
      modelDataset.value = cascaderDataset + " (Classification)"
      modelDatasetSimple.value = cascaderDataset
    }else if(cascaderDataset==="COCO"){
      modelDataset.value = cascaderDataset + " (Detection)"
      modelDatasetSimple.value = cascaderDataset
    }
  ...
  }else if(value[0]=='prune_user' || value[0].type=='pretrained'){
    console.log('这是一个剪枝模型的任务')
    let currentTarget;
    if(value[0]=='prune_user'){
      console.log('剪得是用户的模型')
      currentTarget = value[1]
      modelType.value = "User trained model"
      modelTypeSimple.value = value[1].name
    }else if(value[0].type=='pretrained'){
      console.log('剪的是预训练的模型')
    ...
    console.log("d.path", currentTarget.path)
    modelPath.value = currentTarget.path
    console.log("d.acc", currentTarget.acc)
    modelAcc.value = currentTarget.acc
    console.log("d.params", currentTarget.params)
    modelParams.value = currentTarget.params
    console.log("d.flops", currentTarget.flops)
    modelFlops.value = currentTarget.flops
    // console.log("d.parent.name", value[0].parent.name)
    modelName.value = cascaderModel
    // console.log("d.parent.parent.name", value[0].parent.parent.name)
    ...
    if(cascaderDataset=='CIFAR100' || cascaderDataset=='CIFAR10'){
    ...
    }else if(cascaderDataset=='ImageNet'){
      ImageNetPrunedialogVisible.value = true
      // console.log("d.type", d.type)
      if(cascaderModel.includes('hf')){
        INtype.value = 'hf';
      }else if(cascaderModel.includes('timm')){
        INtype.value = 'timm';
      }else{
        INtype.value = 'cnn';
      }

      if(currentTarget.model_name=='ImageNet_ResNet50_Pretrained' || currentTarget.model_name=='ImageNet_DenseNet121_Pretrained'){
        lr.value = 0.08
      }else if(currentTarget.model_name=='ImageNet_MobileNetV2_Pretrained'){
        lr.value = 0.045
      }else if(currentTarget.model_name=='ImageNet_VGG19-BN_Pretrained' || currentTarget.model_name=='ImageNet_VGG16-BN_Pretrained'){
        lr.value = 0.01
      }
      // radio1.value = 'wosl'
      // criterion.value = ''
      ...
      // if(value[0].type==='pretrained'){
      //   modelType.value = "Publicly pretrained model"
      //   modelTypeSimple.value = "Pretrain"
      // }else if(value[0].type==='usr'){
      //   modelType.value = "User trained model"
      //   modelTypeSimple.value = value[0].name
      // }
      //
      // console.log("d.path", value[0].path)
      // modelPath.value = value[0].path
      // console.log("d.acc", value[0].acc)
      // modelAcc.value = value[0].acc
      // console.log("d.params", value[0].params)
      // modelParams.value = value[0].params
      // console.log("d.flops", value[0].flops)
      // modelFlops.value = value[0].flops
      // console.log("d.parent.name", value[0].parent.name)
      // modelName.value = cascaderModel
      ...
      console.log("d.type", currentTarget.type)
      // if(value[0].type==='pretrained'){
      //   modelType.value = "Publicly pretrained model"
      //   modelTypeSimple.value = "Pretrain"
      // }else if(value[0].type==='usr'){
      ...
      // console.log("d.flops", value[0].flops)
      // modelFlops.value = value[0].flops
      // console.log("d.parent.name", cascaderModel)
      // modelName.value = cascaderModel
      ...
      console.log("标记d.parent.parent.name: ", cascaderDataset)
    }
        ...
        .then((response)=>{
          ...
          for(let i=0; i<originGPUs.length; i++){
            let option = {
              ...
              series : [
                {
                  name:originGPUs[i].name+' GPU information',
                  type: 'pie' ,
                  radius: '70%',
                  data: [
                    {value: originGPUs[i].tot_memory - originGPUs[i].remain_memory,  name:'Used'},
                    {value: originGPUs[i].remain_memory,  name:'Available'},
                  ],
    ...
    console.log('这是未知类型的任务,出错了吧!')
  }

  LeafNodeVisible.value = false
}

var cascader_options = [
  {
    value: 'upload',
    label: 'Upload a raw model',
  },
  {
    value: 'prune_pretrained',
    label: 'Prune a pretrained model',
  },
  {
    value: 'prune_user',
    label: 'Prune a user-trained model',
    disabled: true,
  },
]
...
const SubmitTask = async () => {
  ...
  console.log("localStorage.getItem('userTicket'): ", localStorage.getItem('userTicket'))
  ...
  if (radio1.value != 'sl' && radio1.value != 'wosl') {
  ...
  } else if (radio1.value === 'sl' && (sl_total_epochs.value < 1 || sl_total_epochs.value > 100 || sl_total_epochs.value === undefined || sl_total_epochs.value === '')) {
  ...
  } else if (finetune.value === 'True' && (finetunemode.value === 'DDP')) {
    ElMessage.error("CIFAR do not support DDP")
  } else {
    let modelname = ''
    ...
    if (modelDataset.value === 'CIFAR10 (Classification)') {
      ...
      dataroot = '~/datasets/ILSVRC2012/ILSVRC2012/imagenet'
      dataset = 'imagenet'
    ...
    if (dataset == 'imagenet') {
      if (modelname == 'resnet50') {
        if (finetunemode.value === 'single')
          tempScript = "python /nfs/lhl/Torch-Pruning/benchmarks/main_imagenet.py --mode prune --pretrained --epochs " + epoch.value + " --model " + modelname + " --batch-size " + batchsize.value + " --lr " + lr.value + " --lr-step-size 30 --sl-epochs 30 --sl-lr 0.01 --sl-lr-step-size 10 --prune --cache-dataset --data-path " + dataroot + " --output-dir /nfs/lhl/Torch-Pruning/benchmarks/log/prune  --reg 1e-4 --method " + criterion.value + " --global-pruning --soft-keeping-ratio 0.5 --target-flops " + speedup.value + " --global-pruning --print-freq 100 --workers 8 --finetune " + finetune.value + " --client " + client.value
        else
          tempScript = "torchrun --nproc_per_node=4 /nfs/lhl/Torch-Pruning/benchmarks/main_imagenet.py --mode prune --pretrained --epochs " + epoch.value + " --model " + modelname + " --batch-size " + batchsize.value + " --lr " + lr.value + " --lr-step-size 30 --sl-epochs 30 --sl-lr 0.01 --sl-lr-step-size 10 --prune --cache-dataset --data-path " + dataroot + " --output-dir /nfs/lhl/Torch-Pruning/benchmarks/log/prune  --reg 1e-4 --method " + criterion.value + " --global-pruning --soft-keeping-ratio 0.5 --target-flops " + speedup.value + " --global-pruning --print-freq 100 --workers 8 --finetune " + finetune.value + " --client " + client.value + " --distributed"
        ...
        if (finetune.value === 'False') {
          ...
          tempScript = "torchrun --nproc_per_node=4 /nfs/lhl/Torch-Pruning/transformers/finetune.py --model " + modelPath.value + " --epochs " + epoch.value + " --pruning_type " + criterion.value + " --batch-size " + batchsize.value + " --opt adamw --lr 0.00015 --wd 0.3 --lr-scheduler cosineannealinglr --lr-warmup-method linear --lr-warmup-epochs 0 --lr-warmup-decay 0.033 --amp --label-smoothing 0.11 --mixup-alpha 0.2 --auto-augment ra --clip-grad-norm 1 --ra-sampler --random-erase 0.25 --cutmix-alpha 1.0  --output-dir output/vit_b_16_pruning_taylor_uniform_v2" + " --finetune " + finetune.value + " --client " + client.value + " --speed-up " + speedup.value + "  --cache-dataset --distributed"
        }
        ...
        if (finetune.value === 'False') {
          ...
          tempScript = "torchrun --nproc_per_node=4 /nfs/lhl/Torch-Pruning/transformers/finetune.py --model " + modelPath.value + " --epochs " + epoch.value + " --pruning_type " + criterion.value + " --batch-size " + batchsize.value + " --opt adamw --lr 0.00015 --wd 0.3 --lr-scheduler cosineannealinglr --lr-warmup-method linear --lr-warmup-epochs 0 --lr-warmup-decay 0.033 --amp --label-smoothing 0.11 --mixup-alpha 0.2 --auto-augment ra --clip-grad-norm 1 --ra-sampler --random-erase 0.25 --cutmix-alpha 1.0  --output-dir output/vit_b_16_pruning_taylor_uniform_v2" + " --finetune " + finetune.value + " --client " + client.value + " --speed-up " + speedup.value + " --model_name vit_b_16(hf) --distributed --cache-dataset"
        }
      }
    ...
    if (store.state.access == 0) {
      success = 'Waiting'
      console.log("管理员")
    } else if (store.state.access == 1) {
      success = 'Need Approval'
      console.log("普通用户")
    } else {
      ElMessage.error("Authority error!!")
      return
    }
    ...
    let task_id
    await request.post(`/user/SubmitTrainingHistory`, temp1)
        .then((response) => {
    ...
    if (success === 'Need Approval') { //普通用户需要提交任务
      let temp = {
        ...
        ranking: task_id,
        morfPath: 'pruning task',
        lerfPath: 'N/A', //提交的算法类型
      ...
      await request.post(`/user/SubmitAlgorithm`, temp)
          .then((response) => {
            // console.log("temp1: ", temp1)
            if (response.status === 200) {
              ElMessage.success('Pruning task has been submitted, but need approval.')
            } else {
              ElMessage.error('Sorry, task submission has failed.')
            }
          })
          .catch((errors) => {
            // console.log("temp1: ", temp1)
            ElMessage.success('Sorry, task submission has failed.')
            console.log('error', errors)
          })
...
async function centerNodebyName(datasetName, modelName){
  ...
  const targetGhostCircle = tryNode.filter(function(d) {  // 给圆圈加上红圈特效,新版本只有第一个条件能满足了
  ...
  const targetLink = tryLink.filter(function(d) {  //新版本不会再出现targetLink了,但是不想删
...
function centerNode(source) {
  ...
  var x = -source.y0;
  var y = -source.x0;
  x = x * scale + viewerHeight / 4;  //修改以适应最近的树结构
  // x = x * scale + viewerHeight / 20;
  y = y * scale + viewerHeight / 2;
  d3.select('g').transition()
      .duration(duration)
  ...
  node.select("circle.nodeCircle")
      ...
      .style("fill", function(d) {
        ...
        if(d.type==="upload"){
          ...
          if(d.name==="Model Zoo"){
            console.log("Model Zoo is here")
            return "url(#hl)";
          }else if(d.name==="PowerGrid Dataset"){
            console.log("Grid Model is here")
            return "url(#grid)";
...
async function click(d) {
  ...
  if(d.name!='Model Zoo' && (d.parent.name=='CIFAR100' || d.parent.name=='CIFAR10' || d.parent.name=='COCO' || d.parent.name=='ImageNet' || d.parent.name=='PowerGrid')){
    ...
    //   ElMessage.warning("You need to log in first before submitting your task")
    // }
    LeafNodeVisible.value = true

    cascader_value.value = []

    cascaderDataset = d.parent.name
    cascaderModel = d.name

    let current_key = d.parent.name + "-" + d.name
    let isDisabled = true
    let Currentchildren = []
    let Currentupload = {}
    let Currentpretrained = {}

    current_json = json_dict[current_key];
    console.log("current_json: ", current_json)

    for (const term of current_json){  //不能直接遍历d因为之后会删掉d的子节点们
      ...
      if(term.type==='usr'){
        ...
        //在Currentchildren中增加child
      }else if(term.type==='upload'){
        Currentupload = term
      }else if(term.type==='pretrained'){
        Currentpretrained = term
      }
    }
    ...
    cascader_options = [
      {
        value: Currentupload,
        label: 'Upload a raw model',
        disabled: !store.state.isAut,
      },
      {
        value: Currentpretrained,
        label: 'Prune a pretrained model',
        disabled: !store.state.isAut,
      },
      {
        value: 'prune_user',
        label: 'Prune a user-trained model',
        disabled: !store.state.isAut || isDisabled,
  ...
  centerNode(d);
  if(d.type=='pretrained' || d.type=='usr'){  //这个条件在叶子节点采用级联选择器之后就会作废
    radio1.value = 'wosl'  //要不要sparse learing
    criterion.value = ''  //重要性指标
    batchsize.value = 0
    client155_form.value = 'Bar'
  ...
  }else if(d.type=='upload'){  //这个条件在叶子节点采用级联选择器之后就会作废
    ckpt.value = ''
    ckptUploaded.value = false
    uploadType.value = ''
    modelName.value = d.parent.name
...
onMounted(async () => {
  ...
  if(!store.state.isAut){
    ElMessage.warning("Online pruning is not available since you have not logged in yet.")
  }
  ...
  try {
    ...
    for (const dataset of json.children) {
      for (const network of dataset.children) {
        const key = `${dataset.name}-${network.name}`;
        if(currentAccess!=0&&currentAccess!=1){
          for (const item of network.children){
            network.children = network.children.filter(item => item.type!=='usr');
          }
        ...
        delete network.children;  //删除了之前的叶子节点
      }
    }
