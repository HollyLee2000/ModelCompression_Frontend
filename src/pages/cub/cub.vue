<template>
  <div class="history">
    ...
    >
      ...
      <el-table-column min-width="120" prop="institute" label="institute" />
      <el-table-column min-width="140" prop="morfPath" label="submission type">
        <template #default="props">
          <text v-if="props.row.morfPath==='leaderboard'">algorithm (leaderboard)</text>
          <text v-else-if="props.row.morfPath==='suggestion'">suggestion/bug/issue</text>
          <text v-else>{{props.row.morfPath}}</text>
        </template>
      </el-table-column>
      <el-table-column min-width="120" prop="name" label="algorithm name">
        <template #default="props">
          <text v-if="props.row.morfPath==='leaderboard'">{{props.row.name}}</text>
          <text v-else>N/A</text>
        </template>
      </el-table-column>
      <el-table-column min-width="120" prop="lerfPath" label="algorithm type" >
        <template #default="props">
          <text v-if="props.row.morfPath==='leaderboard'">{{props.row.lerfPath}}</text>
          <text v-else>N/A</text>
        </template>
      </el-table-column>
...
<!--      <el-table-column min-width="100" prop="ranking" label="ranking" sortable="sortable" />-->
      ...
      <el-table-column min-width="40" label="Infos" type="expand">
        <template #default="props">
          <div m="4">
...
<!--            <div>-->
              <p m="t-0 b-2" v-if="props.row.morfPath==='leaderboard'">Algorithm information: {{ props.row.info }}</p>
              <p m="t-0 b-2" v-else-if="props.row.morfPath==='suggestion'">User suggestion: {{ props.row.info }}</p>
              <p m="t-0 b-2" v-else>User script: {{ props.row.info }}</p>

<!--              <text v-if="props.row.morfPath==='leaderboard'">Algorithm information: {{ props.row.info }}</text>-->
<!--              <text v-else>User suggestion: {{ props.row.info }}</text>-->
<!--            </div>-->

<!--            <div>-->
              <p m="t-0 b-2" v-if="props.row.morfPath==='leaderboard'">Algorithm statistics / Paper link: {{ props.row.email }}</p>
<!--              <text v-else>User suggestion: {{ props.row.info }}</text>-->
<!--            </div>-->
<!--            -->
<!--            <text m="t-0 b-2">Algorithm information: {{ props.row.info }}</text>-->
<!--            <br>-->
<!--            <text m="t-0 b-2">Algorithm statistics / Paper link: {{ props.row.email }}</text>-->
      ...
      <el-table-column label="Decisions" min-width="160">
        <template #default="props">
          ...
          <div v-if="props.row.status==='In Process'&&(props.row.morfPath==='leaderboard'||props.row.morfPath==='pruning task'||props.row.morfPath==='upload task')">
            <el-button
                size="small"
          ...
          <div v-else-if="props.row.morfPath==='suggestion'">
            <el-text class="mx-1" type="info" style="color: #4C4C4C; font-weight: bold">N/A</el-text>
          </div>
    ...
    <span>
      Are you sure to approve this submission? It means that this algorithm will be added to the leaderboard.
    </span>
      <template #footer>
      <span class="dialog-footer">
  ...
  request.post('/history/findAll',{
    username: store.state.username,
      ...
      .then((response)=>{
        ...
        let originData=response.data.data.infoInfos
